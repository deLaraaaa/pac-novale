...
class _MarketGainInfoScreenState extends State<MarketGainInfoScreen> {
  ...
  void initState() {
    ...
    ['novosClientes', 'clientesPerdidos', 'prospectados'].forEach((field) {
    ...
    if (response.statusCode == 200) {
        ...
        if (item.containsKey('date') && item['date'] != null) {
          try {
            ...
            if (!itemDate.isBefore(startDate) && !itemDate.isAfter(endDate)) {
              ...
              totalNovosClientes += _parseToInt(item['novosClientes']);
              totalClientesPerdidos += _parseToInt(item['clientesPerdidos']);
              totalProspectados += _parseToInt(item['prospectados']);
            }
          } catch (e) {
      ...
      setState(() {
        ...
        if (showAverage && count > 0) {
          values['novosClientes'] = (totalNovosClientes / count).round();
          values['clientesPerdidos'] = (totalClientesPerdidos / count).round();
          values['prospectados'] = (totalProspectados / count).round();
        } else {
          values['novosClientes'] = totalNovosClientes;
          values['clientesPerdidos'] = totalClientesPerdidos;
          values['prospectados'] = totalProspectados;
        }
      });
    } else {
