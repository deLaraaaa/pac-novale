...
const updateCompanyInfo = async (req, res) => {
  ...
  try {
    ...
    if (type == "engagement") {
      ...
      const newMarketGain = {
        date: date,
        novosClientes: values.novosClientes || 0,
        clientesPerdidos: values.clientesPerdidos || 0,
        prospectados: values.prospectados || 0,
      };
      // Adicionar ou atualizar o documento com o ID igual ao ano e mês
      await marketGainRef.doc(yearMonth).set(newMarketGain);
      // Retornar sucesso
      res.status(200).json({
